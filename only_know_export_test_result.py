...
tf.app.flags.DEFINE_float(
    'new_whale_prob', None,
    'prob to treat the image as a new_whale')
...
def main(_):
  ...
  for f in all_files:
      if f[-4:] == '.jpg':
          all_images.append(os.path.join('/home/westwell/Desktop/dolores_storage/'
                                         'humpback_whale_identification/data/all/test/',
                                         f))
  # train new_whale images
  train_new_whale_all_files = []
  with open('/home/westwell/Desktop/dolores_storage/'
            'humpback_whale_identification/data/all/new_whale.csv', 'r') as fid:
      for l in fid.readlines():
          if 'Image' in l:
              continue
          l = l.strip().split(',')
          train_new_whale_all_files.append(
              os.path.join(
                  '/home/westwell/Desktop/dolores_storage/'
                  'humpback_whale_identification/data/all/train', l[0]))
  ...
  for ckpt_file in checkpoints_iterator(
      ...
      with tf.Session(config=sess_cfg) as sess:
          ...
          # get train_new_whale_prob
          train_new_whale_prob = None
          if FLAGS.new_whale_prob is not None:
              new_whale_prob = FLAGS.new_whale_prob
          else:
              for i, im_file_path in enumerate(train_new_whale_all_files):
                  print(i, im_file_path)
                  one_im = cv2.imread(im_file_path)
                  one_im = cv2.cvtColor(one_im, cv2.COLOR_BGR2RGB)
                  one_prob = sess.run(probabilities, feed_dict={input_image: one_im})
                  if train_new_whale_prob is None:
                      train_new_whale_prob = one_prob
                  else:
          ...
          output_file_path = os.path.join(
              FLAGS.output_dir, 'submission_%s_%.4f.csv' % (time.time(), new_whale_prob))
          ...
          for i, im_file_path in enumerate(all_images):
              ...
              one_im = cv2.cvtColor(one_im, cv2.COLOR_BGR2RGB)
              one_prob = sess.run(probabilities, feed_dict={input_image: one_im})
              one_prob = np.hstack((one_prob,[[new_whale_prob]]))
              one_prob = np.squeeze(one_prob)
              one_top_k_idx = np.argsort(one_prob, axis=-1)[::-1]
              one_top_k_idx = one_top_k_idx[:FLAGS.top_k]
